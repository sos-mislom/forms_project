...
const AddContainer = ({questionsAndFields = [], onAddQuestionOrField, onDeleteQuestionOrField, onUpdateQuestionOrField, isUserAnswer = false}) => {
        ...
        switch (item.type) {
            ...
            case 'video':
                return  <div className="uploaded">
                            ...
                            <button className='delete-uploaded' onClick={() => onDeleteQuestionOrField(item.id)}>
                                {!isUserAnswer && <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            case 'text':
                return  <FieldText key={item.id } id={item.id} onUpdate={onUpdateQuestionOrField} onDelete={onDeleteQuestionOrField} fieldText={item.textField} isUserAnswer={isUserAnswer}/>;
            case 'title':
                return  <FieldTitle key={item.id } id={item.id} onUpdate={onUpdateQuestionOrField} fieldDescription={item.descriptionField} titleField={item.titleField}/>;
            case 'single':
                return <QuestionSingle key={item.id } id={item.id} onDelete={onDeleteQuestionOrField} onUpdate={onUpdateQuestionOrField} questionText={item.textQuestion} initialOptions={item.options} initialCorrectAnswers={item.correctAnswers} userAnswer={item.userAnswer} isUserAnswer={isUserAnswer} currentScore={item.score}/>;
            case 'multiple':
                return <QuestionMultiple key={item.id} id={item.id} onDelete={onDeleteQuestionOrField} onUpdate={onUpdateQuestionOrField} questionText={item.textQuestion} initialOptions={item.options} initialCorrectAnswers={item.correctAnswers} userAnswer={item.userAnswer} isUserAnswer={isUserAnswer} currentScore={item.score}/>;
            case 'short':
                return <QuestionShort key={item.id} id={item.id} onDelete={onDeleteQuestionOrField} onUpdate={onUpdateQuestionOrField} questionText={item.textQuestion} initialCorrectAnswers={item.correctAnswers} isUserAnswer={isUserAnswer} userAnswer={item.userAnswer} currentScore={item.score}/>;
            default:
                return null;
        }
    };
    return (
        <div className="add-container">
            ...
            {!isUserAnswer && <DropdownQuestions onAddQuestion={onAddQuestionOrField} />}
        </div>
    );
};
